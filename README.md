...
This website adopts a intuitive and minimalistic design with all the content displayed in a linear layout. The content includes data retrieved from the Application Programming Interface (API) as well as other relevent information. In terms of colour choice, I used an Achromatic Colour Scheme. For light mode appearance, a very light grey (#edf0f1) was chosen for the background colour of most content except the user input section, and white for the website background. As for dark mode appearance, different tones of dark grey were chosen to replace the background colours of light mode. 
